...
"""
MedLLM Fresh Training - Avoiding PyTorch Checkpoint Issues
Start completely fresh with optimized settings
"""
...
try:
    ...
    os.system("pip install -q unsloth transformers datasets trl wandb")
    from unsloth import FastLanguageModel
    from datasets import Dataset, load_dataset, concatenate_datasets
    from transformers import TrainingArguments, set_seed, TrainerCallback
    from trl import SFTTrainer
...
def main():
    """Main execution"""
    parser = argparse.ArgumentParser(description="Fresh MedLLM Training")
    ...
    wandb.init(
        ...
        config={
            ...
            "hardware": "RTX 3070Ti",
            "training_type": "fresh_start",
            "optimizations": "All fixes applied from start",
        }
    )
    
    # Create and run fresh trainer
...
if __name__ == "__main__":
    logger.info("🏥 MedLLM Fresh Training - Clean Start")
    main()
